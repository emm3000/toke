...
class LoginApplicantViewModel(private val repository: AuthRepository) : ViewModel() {
    ...
    var state by mutableStateOf(LoginApplicantUiState())
        private set
    ...
    fun onAction(event: LoginApplicantEvent) {
        when (event) {
            is LoginApplicantEvent.EmailChanged -> state = state.copy(email = event.email)
            is LoginApplicantEvent.PasswordChanged -> state = state.copy(password = event.password)
            is LoginApplicantEvent.Login -> login()
            LoginApplicantEvent.DismissError -> state = state.copy(error = null)
        }
    }
